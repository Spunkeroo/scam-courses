const Votes = {
  getKey(type, id) { return `scamcourses_vote_${type}_${id}`; },
  getCountKey(type, id) { return `scamcourses_votes_${type}_${id}`; },
  getUserVote(type, id) { return localStorage.getItem(this.getKey(type, id)); },
  ...
  render(type, id) {
    ...
    return `
      <div class="vote-box" data-vote-type="${type}" data-vote-id="${id}">
        ...
        <button class="vote-btn ${userVote === 'up' ? 'active-up' : ''}" onclick="Votes.handleVote('${type}', '${id}', 'up')">&#9650; ${counts.up}</button>
        <span class="vote-count">${score}</span>
        <button class="vote-btn ${userVote === 'down' ? 'active-down' : ''}" onclick="Votes.handleVote('${type}', '${id}', 'down')">&#9660; ${counts.down}</button>
      </div>
    `;
  },
  ...
  handleVote(type, id, direction) {
    ...
    if (container) container.outerHTML = this.render(type, id);
  }
};